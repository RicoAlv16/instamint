...
export class SearchMinterService {
    ...
    async createMinter(minterPost: CreateMinterPostDto): Promise<CreateMinterPostDto> {
        try {
            const savedMinter = await this.minterPostRepository.save(minterPost);
            return savedMinter;
        } catch (error) {
            throw new HttpException(`Error saving Minter: ${error.message}`, HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }

    async createTeaBag(teaBagPost: CreateTeaBagPostDto): Promise<CreateTeaBagPostDto> {
        try {
            const savedTeaBag = await this.teaBagPostRepository.save(teaBagPost);
            return savedTeaBag;
        } catch (error) {
            throw new HttpException(`Error saving TeaBag: ${error.message}`, HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
