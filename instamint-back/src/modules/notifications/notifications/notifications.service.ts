...
export class NotificationsService {
  ...
  async createNotifications(
    notificationsPost: CreateNotificationsPostDto
  ): Promise<CreateNotificationsPostDto> {
    try {
      const savedNotifications =
        await this.notificationsPostRepository.save(notificationsPost);
      return savedNotifications;
    } catch (error) {
      throw new HttpException(
        `Error saving Notifications: ${error.message}`,
  ...
  async getNotificationsByMinter(
    idMinter: number
