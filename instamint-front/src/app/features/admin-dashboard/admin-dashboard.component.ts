...
export class AdminDashboardComponent {
  ...
  onMinterIdInput(event: Event): void {
    ...
    this.minterId = inputElement.value ? parseInt(inputElement.value, 10) : null;
  }

  onTeaBagIdInput(event: Event): void {
    const inputElement = event.target as HTMLInputElement;
    this.teaBagId = inputElement.value ? parseInt(inputElement.value, 10) : null;
  }
  ...
  disableMinter(): void {
    if (this.minterId !== null) {
      this.adminService.disableMinter(this.minterId).subscribe({
        ...
        error: (error) => alert(`Failed to disable Minter account ${this.minterId}: ${error.message}`)
      });
    } else {
  ...
  deleteMinter(): void {
    if (this.minterId !== null) {
      this.adminService.deleteMinter(this.minterId).subscribe({
        ...
        error: (error) => alert(`Failed to delete Minter account ${this.minterId}: ${error.message}`)
      });
    } else {
      alert('Please enter a valid Minter ID.');
    }
  ...
  deleteTeaBag(): void {
    if (this.teaBagId !== null) {
      this.adminService.deleteTeaBag(this.teaBagId).subscribe({
        ...
        error: (error) => alert(`Failed to delete Tea Bag ${this.teaBagId}: ${error.message}`)
      });
    } else {
  ...
  deleteNft(): void {
    if (this.nftId !== null) {
      this.adminService.deleteNft(this.nftId).subscribe({
        ...
        error: (error) => alert(`Failed to delete NFT ${this.nftId}: ${error.message}`)
      });
    } else {
